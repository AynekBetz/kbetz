...
export default function Dashboard() {
  ...
  const processGames = (incomingGames) => {
    ...
    const updated = cleanGames.map((g) => {
      ...
      if (movement === "up") {
        reasons.push("📈 Positive line movement");
      } else if (movement === "down") {
        reasons.push("📉 Market moving against this side");
      } else {
        reasons.push("➖ Stable betting market");
      }
      ...
      if (winProbability >= 65) {
        reasons.push("🎯 Strong projected win probability");
      }
